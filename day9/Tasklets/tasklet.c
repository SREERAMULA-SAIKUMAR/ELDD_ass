...
int TLL_open(struct inode *inode,struct file *filp);
ssize_t TLL_read(struct file *filp,char __user *ubuff,size_t count,loff_t *offp);
ssize_t TLL_write(struct file *filp, const char __user *ubuff,size_t count,loff_t *offp);
int TLL_release(struct inode *inode,struct file *filp);
...
{
    if((alloc_chrdev_region(&dev,99,1,"TLL"))<0)
    {
    ...
    if(request_irq(IRQ_NO,irq_handler,IRQF_SHARED,"TLL",(void *)(irq_handler)))
    {
